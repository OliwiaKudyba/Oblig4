...
def save_car(make, model, reg, year, status, location):
    with _get_connection().session() as session:
        cars = session.run("MERGE (a:Car{make: $make, model: $model, reg: $reg, year: $year, status:$status, location$location}) RETURN a;", 
...
def update_car_status(reg, new_status):
    with _get_connection().session() as session:
        session.run("MATCH (car:Car {reg: $reg} ) set car.statusOfCar = $new_status",
